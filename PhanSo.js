...
const basisSetOfPaths = findBasisSetOfPaths(controlFlowGraph.control, 1, controlFlowGraph.nodeEnd);
const testCases = [...generateTestCase(controlFlowGraph, basisSetOfPaths)];


const WriteToExcel = async(testCases, basisSetOfPaths)=>{
  let rowNumber=4;
  const workbookData= await workbook.xlsx.readFile(process.env.FILE_EXCEL_PATH);
  const worksheet = workbook.getWorksheet(process.env.WORK_SHEET_NAME);
  for(let i = 0; i < testCases.length; i++){
    console.log(basisSetOfPaths[i].join("-> "));
    console.log("a= "+testCases[i].a+" b= "+testCases[i].b+" | Output: "+ phanSo(testCases[i].a, testCases[i].b));
    worksheet.getCell(`A${rowNumber}`).value=basisSetOfPaths[i].join("-> ");
    worksheet.getCell(`B${rowNumber}`).value=testCases[i].a;
    worksheet.getCell(`C${rowNumber}`).value=testCases[i].b;
    worksheet.getCell(`D${rowNumber}`).value = phanSo(testCases[i].a, testCases[i].b);
    rowNumber++;
  }               
...
WriteToExcel(testCases,basisSetOfPaths);